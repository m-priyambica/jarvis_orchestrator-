...
class DashboardServer:
    ...
    # ── broadcast ────────────────────────────────────────────────────────
    #
    # Transport-agnostic on purpose: msg is JSON-serialized as-is, callers
    # in main.py own the schema. Known message types in current use:
    #   {"type": "status", "state": "active" | "sleeping"}
    #   {"type": "agent_roll_call", "agents": [...], "tasks": [...]}
    #     — sent from JarvisLive._render_jarvis_os_panel() on every JARVIS OS
    #     panel refresh (~10s cadence). Each entry in `agents` is one of the
    #     nine supervisor rows from core.jarvis_os_bridge.get_agent_roll_call()
    #     — id, name, status, last_task, and current_file_reference (a
    #     best-effort file path / URL string, '' when idle or unknown; see
    #     orchestrator._extract_reference) — this is what the 3D UI reads to
    #     render which file/URL each agent node is actively touching.
