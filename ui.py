...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "J.A.R.V.I.S", parent=None):
        ...
        # Agent ring — the 9 supervisors orbiting the core. Fed in-process by
        # set_roll_call() from get_agent_roll_call(); deliberately no socket,
        # this runs in the same process as the orchestrator bridge.
        self._agents: list[dict] = []
        self._orbit  = 0.0
        self._agent_pulse: dict[str, float] = {}   # id -> 0..1 flash on status change
        self._agent_prev: dict[str, str] = {}      # id -> last seen status
    ...
    def set_roll_call(self, roster: list[dict]):
        """Update the orbiting agent ring from get_agent_roll_call()'s rows
        ({id, name, status, current_file_reference, last_task}). Flashes any
        agent whose status changed since the last call so a team picking up
        work is visible the moment it happens, not just on the next glance.
        """
        for a in roster or []:
            aid, st = a.get("id", ""), a.get("status", "idle")
            if self._agent_prev.get(aid, st) != st:
                self._agent_pulse[aid] = 1.0
            self._agent_prev[aid] = st
        self._agents = list(roster or [])
    ...
    def _step(self):
        ...
        # agent ring drifts slowly; any team that just changed status decays
        # its flash back to rest over ~1s
        self._orbit = (self._orbit + (0.42 if self.speaking else 0.20)) % 360
        if self._agent_pulse:
            self._agent_pulse = {
                k: v - 0.016 for k, v in self._agent_pulse.items() if v > 0.016
            }
    ...
    # Vertical squash of the orbit ellipse — this is what sells the ring as a
    # tilted 3D orbit rather than a flat circle of dots.
    _ORBIT_TILT = 0.34

    _AGENT_COLOR = {"busy": C.ACC, "blocked": C.RED}

    def _agent_nodes(self, cx: float, cy: float, fw: float) -> list[tuple]:
        """Project each agent onto the tilted orbit. Returns tuples of
        (depth, x, y, agent) where depth 0 = far side, 1 = near side.
        """
        n = len(self._agents)
        if n == 0:
            return []
        ...
        for i, a in enumerate(self._agents):
            th  = math.radians(self._orbit + i * (360.0 / n))
            s   = math.sin(th)
            out.append((
                (1.0 - s) / 2.0,
                cx + R * math.cos(th),
                cy - R * self._ORBIT_TILT * s,
                a,
            ))
        return out

    def _draw_agent_nodes(self, p: QPainter, nodes: list[tuple], fw: float,
                          cx: float = 0.0, cy: float = 0.0):
        """Draw one depth-sorted slice of the ring. Near nodes are bigger,
        brighter, and labelled; far nodes recede into the background.
        """
        p.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        for depth, x, y, a in sorted(nodes, key=lambda t: t[0]):
            status = a.get("status", "idle")
            base   = self._AGENT_COLOR.get(status, C.PRI_DIM)
            if self.muted:
                base = C.MUTED_C

            # perspective: 0.55x size / 45% alpha at the back, full at the front
            scale = 0.55 + 0.45 * depth
            alpha = int((90 + 165 * depth) * (0.45 if status == "idle" else 1.0))
            alpha = max(0, min(255, alpha))
            r     = fw * 0.021 * scale

            flash = self._agent_pulse.get(a.get("id", ""), 0.0)
            if flash > 0:
                fr = r * (1.0 + 2.2 * flash)
                p.setPen(QPen(qcol(base, int(alpha * flash)), 1.2))
                p.setBrush(Qt.BrushStyle.NoBrush)
                p.drawEllipse(QPointF(x, y), fr, fr)

            # halo + core dot
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(qcol(base, alpha // 4)))
            p.drawEllipse(QPointF(x, y), r * 2.1, r * 2.1)
            p.setBrush(QBrush(qcol(base, alpha)))
            p.drawEllipse(QPointF(x, y), r, r)

            # tether back to the core, brightest on the teams actually working
            p.setPen(QPen(qcol(base, int(alpha * (0.45 if status == "busy" else 0.14))), 1))
            p.setBrush(Qt.BrushStyle.NoBrush)
            gap = fw * 0.33          # stop short of the core so it stays clean
            dx, dy = x - cx, y - cy
            dist = math.hypot(dx, dy) or 1.0
            if dist > gap:
                p.drawLine(QPointF(cx + dx / dist * gap, cy + dy / dist * gap),
                           QPointF(x - dx / dist * r, y - dy / dist * r))

            if depth < 0.42:
                continue   # too far back to label without cluttering the core

            name = (a.get("name") or a.get("id") or "?").split()[0].upper()
            p.setPen(QPen(qcol(base, alpha), 1))
            p.drawText(QRectF(x - 60, y + r + 3, 120, 12),
                       Qt.AlignmentFlag.AlignHCenter, name)

            ref = (a.get("current_file_reference") or "").strip()
            if ref and status == "busy":
                ref = ref.rsplit("/", 1)[-1].rsplit("\\", 1)[-1][:22]
                p.setPen(QPen(qcol(C.TEXT_DIM, alpha), 1))
                p.drawText(QRectF(x - 70, y + r + 14, 140, 12),
                           Qt.AlignmentFlag.AlignHCenter, f"↳{ref}")
    ...
    def paintEvent(self, _):
        ...
        # agent ring — far half first so the core occludes it (the depth cue
        # that makes the orbit read as 3D); near half is drawn after the face.
        _nodes = self._agent_nodes(cx, cy, fw)
        if _nodes:
            orb_r = fw * 0.44
            p.setPen(QPen(qcol(C.PRI, max(28, min(110, int(self._halo * 0.9)))), 1))
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.drawEllipse(QRectF(cx - orb_r, cy - orb_r * self._ORBIT_TILT,
                                 orb_r * 2, orb_r * self._ORBIT_TILT * 2))
            self._draw_agent_nodes(p, [n for n in _nodes if n[0] < 0.5], fw, cx, cy)
        ...
        # agent ring — near half, in front of the core
        if _nodes:
            self._draw_agent_nodes(p, [n for n in _nodes if n[0] >= 0.5], fw, cx, cy)
...
class MainWindow(QMainWindow):
    ...
    _roll_sig       = pyqtSignal(list)       # agent roll call → HUD orbit ring
    ...
    def __init__(self, face_path: str):
        ...
        self._roll_sig.connect(self.hud.set_roll_call)
    ...
    def _desktop_shortcut_path(self) -> Path:
        """Where `_create_desktop_shortcut` writes on this platform — used to
        detect whether one already exists, without creating anything."""
        desktop = self._get_desktop_dir()
        _os = platform.system()
        if _os == "Windows":
            return desktop / "J.A.R.V.I.S.lnk"
        elif _os == "Darwin":
            return desktop / "J.A.R.V.I.S.app"
        return desktop / "J.A.R.V.I.S.desktop"

    def _has_desktop_shortcut(self) -> bool:
        try:
            return self._desktop_shortcut_path().exists()
        except Exception:
            return False
...
class JarvisUI:
    ...
    def has_desktop_shortcut(self) -> bool:
        return self._win._has_desktop_shortcut()
    ...
    def set_roll_call(self, roster: list):
        """Push the 9-supervisor roll call onto the HUD's orbit ring. Safe to
        call from the asyncio/worker threads — goes through a Qt signal like
        set_state does, since Qt widgets are main-thread only.
        """
        self._win._roll_sig.emit(list(roster or []))
